...
| **S4 成文** | 参数回填：Z/公式/速率**逐个从账本液面导出**；按形态成文；wfp 形态走 **Reference-first 产线惯例对齐**（09 §八） | 参数：`13`→`11`→（兜底`05`）；形态：`02` spredo / `09` §八 wfp（惯例对齐）/ `04` XML | WD 交付：**`wfp_builder` 直接生成（首选，2026-09-10 用户定）**；`spredo转wfp_生成器`+`wfp后处理_模板` 仅用于已有 spredo 脚本的兼容转换 | 每个 Z 可溯源到账本；编译过；**结构对齐检查（09 §八.3）过** |
...
- S1：读法先定通道（`00` §〇 双通道纪律：数字走文本抽取、流程图/结构走视觉读；
  开读前先问版本以哪版为准）
- S4 形态（2026-09-10 用户定）：**WD 交付 = `wfp_builder` 直接生成**（API 直构活动树：block/parallel/loop/dely/pcr_stop，disp_name 语义命名生成时即写，z 支持循环变量公式串；样板 `scripts/wfp_builder_示例.py`（自证：重生成参考工程并逐字段比对）），不再走『先写 spredo 再转换器』——该路径仅保留给『已有 spredo 脚本要转 wfp』的场合（转后仍过 `wfp后处理_模板`）。最小骨架只是字节锚点，规范化基准=现场 Reference 工程（skill 自带 scripts/参考工程_结构示例.wfp）。语法技巧清单（09 §八 + 13）：parallel_block 开机并行、非温控等待用 dely()（禁 PCR 空跑计时）、收尾 pcr_stop_heating、回打平衡、悬空打乙醇+TipTouch 靠壁、分装源井每列 Z 表/公式、8 头 dict 必带 Tips:8、循环 range 单参+表达式偏移、小体积 mix 双约束。builder 产物仍过 09 §八.3 结构对齐检查 + 账本复核
...
| 方向 | 方法 |
|---|---|
| **Python → WD** | `scripts/spredo转wfp_生成器.py <脚本.py> <模板.wfp> <输出.wfp> [工程名] [固定PCR方法名] [--sw 1.9.0.395]`——AST 自动转换，已实战（444 节点工程）；模板用仓库自带 `scripts/最小骨架模板.wfp`（默认 395 版=作者现场实机，`最小骨架wfp_合成器.py --sw` 可切他版）或用户任一单换台工程；限制见脚本头部注释（需单换台结构、require2 不转换）。**使用操作（模板/命令/验证/排错）见 `docs/生成WFP文件操作指南.md`** |
...
| ⑧ 环境基线 | **开场必问配置号（1–15，对号 `references/01` §〇 全系配置表；答不上先给 01 §〇 特点描述帮其对号）**；现场软件版本号（**硬规则：wfp 一律按 1.9.0.395 串生成**——395 文件在 398 软件可开、反向必报"项目文件版本高"；**必实证**：要一个现场现役 .wfp 看版本串并留作 Reference 只读基准——09 §八；skill 自带 `scripts/参考工程_结构示例.wfp` 结构金标准；）；温控模块几台；**24 板位实际可放板清单**（尤其 PCR/磁架/温控位——配置 2 实机 2026-09-09 确认：PCR 模块 ×3 但 POS9/10 封闭放不了板，可放板的 PCR 位仅 POS11） | 版本与板位清单写死进交付说明；**台面位有存疑直接问现场，勿单信档案** |
...
| 工具 | scripts/ | spredo→wfp 生成器（+活动 schema）；**wfp 程序化直改与全流程液量账本**（09 §七、13 §十）；wfp 后处理（Block 分组+语义命名+版本归一，09 §八）、全面自检、英文版、说明书生成器（模板类：改 CONFIG 后运行；台账见 `scripts/README.md`） |
